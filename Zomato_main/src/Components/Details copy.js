...
setAddress = (i) => {
  ...
  console.log(menuItems)
    if (menuItems && menuItems.length > 0) {
      const totalCost = menuItems.reduce((acc, item) => acc + parseFloat(item.price), 0);
      return (totalCost / menuItems.length).toFixed(2);
    } else {
      return 0; // Handle the case where there are no menu items
    }
  }
  render() {
    const { restaurant, menuItems, menuModalOpen, subtotal, galleryModal, formModal, averageCost } = this.state;
    return (
      <div>
            ...
            <div className='tab'>
              <input type='radio' id='tab-1' name= 'tab_troup' checked/>
                ...
                <div className='content'>
                  ...
                  <p className='titleText'>₹{this.calculateAverageCost()}{averageCost}(approx.)</p>
