...
class Header extends React.Component {
    ...
    componentDidMount() {
        // Check cookies for saved user data
        const storedUser = Cookies.get('User');
        if (storedUser) {
          try {
            // If user data is found, update the component state
      ...
      componentDidUpdate(prevProps, prevState) {
        // Save user data to cookies when it changes
        if (this.state.User !== prevState.User) { // Use 'User' instead of 'user'
          Cookies.set('User', JSON.stringify(this.state.User)); // Use 'User' instead of 'user'
        }
      }
                        ...
                        <div className="position-absolute float-end" style={{ marginLeft: "50em" }}>
                            {!User ? (
                                ...
                                <form className="d-flex mt-2">
                                    ...
                                    {console.log(User)}
                                    <button type="button" className="btn btn-outline-light ml-2 px-3 accountLogOut" onClick={this.logout}>Logout</button>
                                </form>
                            )}
                        </div>
                    ) : (
